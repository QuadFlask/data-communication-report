...
public class EvenParityGeneratorAndCheckerTest {
	...
	public void evenParityCheckOf110_bit() {
		...
		compare(EvenParityChecker.extract(dataword), 1, 1);
		dataword = toBits(1, 0, 1);
		compare(EvenParityChecker.extract(dataword), 1, 0);
	}

	@Test
	public void evenParityCheckOf11_shouldError_bit() {
		Bit[] dataword = toBits(1, 1, 1);
		assertThat(EvenParityChecker.extract(dataword), IsNull.nullValue());
	}
	...
	public void evenParityCheckOf110() {
		...
		compare(EvenParityChecker.extract(dataword.values()), 1, 1);
		dataword = new BitPattern(1, 0, 1);
		compare(EvenParityChecker.extract(dataword.values()), 1, 0);
	}

	@Test
	public void evenParityCheckOf11_shouldError() {
		BitPattern dataword = new BitPattern(1, 1, 1);
		assertThat(EvenParityChecker.extract(dataword.values()), IsNull.nullValue());
	}
